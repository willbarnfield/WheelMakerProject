function [R,mass,rims,N,rim_no] = CustomConfig3
    ...
    % end
        ...
        if strcmp(choice,'p')
            ...
            createPlot3(R,N,mass,massChoice);

            fprintf("\nThe current position of mass %i is [x,y,z] = [%i,%i,%i]\n",massChoice,mass(massChoice).position_x,mass(massChoice).position_y,mass(massChoice).position_z);
            pos = input("Please enter the new position in vector form [x,y,z]: ");
            mass(massChoice).position_x = pos(1);
            mass(massChoice).position_y = pos(2);
            mass(massChoice).position_z = pos(3);
            maxPos = max(abs(pos(1)),abs(pos(2)));
            maxPos = max(maxPos,abs(pos(3)));
