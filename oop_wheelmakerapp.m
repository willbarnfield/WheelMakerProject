...
while (~(strcmp(type,'standard') || strcmp(type,'cheby') || strcmp(type,'custom') || strcmp(type,'import') || strcmp(type, 'picture')))
    ...
    prompt = {'Enter the config you would like to use'};
    dlgtitle = 'config';
    fieldsize = [1 40];
    definput = {'standard'};
...
if strcmp(type,'standard')
    ...
    prompt = {'How many masses would you like to put on each rim?'};
    dlgtitle = 'config';
    fieldsize = [1 40];
    definput = {'20'};
    answer = inputdlg(prompt,dlgtitle,fieldsize,definput,opts);
    ...
    prompt = {'How many rims would you like to have?'};
    dlgtitle = 'config';
    fieldsize = [1 40];
    definput = {'2'};
    answer = inputdlg(prompt,dlgtitle,fieldsize,definput,opts);
    rim_no = str2num(answer{1});
...
for i=1:N
    ...
    if ismember(i, mass(N).connections)
